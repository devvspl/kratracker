...
<div x-data="{
    ...
    async submit() {
        ...
        const url=isEdit?'/masters/application-modules/'+this.form._id:'/masters/application-modules';
    ...
    async deleteItem() {
        this.loading=true;
        try {
            const res=await fetch('/masters/application-modules/'+this.deleteId,{method:'DELETE',headers:{'X-CSRF-TOKEN':document.querySelector('meta[name=csrf-token]').content}});
            const data=await res.json();
