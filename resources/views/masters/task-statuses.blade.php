...
@php $json = json_encode($statuses->toArray()); @endphp

<div x-data="{
    items: {{ $json }},
    showModal: false, showDelete: false, mode: 'create', loading: false, deleteId: null,
    form: { name:'', color_class:'slate', sort_order:0, is_active:true },

    openCreate() { this.mode='create'; this.form={ name:'', color_class:'slate', sort_order:0, is_active:true }; this.showModal=true; },
    openEdit(id) {
        this.mode='edit';
        const item=this.items.find(i=>i.id===id);
        if(!item) return;
        this.form={ name:item.name, color_class:item.color_class, sort_order:item.sort_order, is_active:item.is_active, _id:id };
        this.showModal=true;
    },
    ...
    async submit() {
        ...
        const url=isEdit?'/masters/task-statuses/'+this.form._id:'/masters/task-statuses';
        const body={ name:this.form.name, color_class:this.form.color_class, sort_order:this.form.sort_order, is_active:this.form.is_active };
        try {
    ...
    async deleteItem() {
        this.loading=true;
        try {
            const res=await fetch('/masters/task-statuses/'+this.deleteId,{method:'DELETE',headers:{'X-CSRF-TOKEN':document.querySelector('meta[name=csrf-token]').content}});
            const data=await res.json();
    ...
    <div class="flex items-center justify-between mb-5">
        <div><h2 class="text-base font-bold text-slate-800">Task Status Master</h2><p class="text-xs text-slate-500 mt-0.5">Manage task status options</p></div>
        <button @click="openCreate()" class="px-3 py-2 bg-teal-600 text-white text-xs font-medium rounded-lg hover:bg-teal-700 flex items-center gap-1.5">
            <svg class="w-3.5 h-3.5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 4v16m8-8H4"/></svg> Add Status
        </button>
    </div>
        ...
        <table class="w-full text-sm">
            ...
            <tbody class="divide-y divide-slate-100">
                ...
                <tr class="hover:bg-slate-50">
                    ...
                    <td class="px-4 py-3">
                        <span class="inline-flex items-center gap-1.5 px-2 py-0.5 rounded text-xs font-medium bg-{{ $status->color_class }}-100 text-{{ $status->color_class }}-700">
                            <span class="w-2 h-2 rounded-full bg-{{ $status->color_class }}-500"></span>{{ ucfirst($status->color_class) }}
                        </span>
                    </td>
                    <td class="px-4 py-3 text-slate-600">{{ $status->sort_order }}</td>
                    <td class="px-4 py-3"><span class="px-2 py-0.5 text-xs rounded-full font-medium {{ $status->is_active ? 'bg-green-100 text-green-700' : 'bg-slate-100 text-slate-500' }}">{{ $status->is_active ? 'Active' : 'Inactive' }}</span></td>
                    <td class="px-4 py-3">
                        <div class="flex items-center gap-2">
                            <button @click="openEdit({{ $status->id }})" class="p-1 text-slate-400 hover:text-blue-600 rounded"><svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M11 5H6a2 2 0 00-2 2v11a2 2 0 002 2h11a2 2 0 002-2v-5m-1.414-9.414a2 2 0 112.828 2.828L11.828 15H9v-2.828l8.586-8.586z"/></svg></button>
                            <button @click="confirmDelete({{ $status->id }})" class="p-1 text-slate-400 hover:text-red-600 rounded"><svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M19 7l-.867 12.142A2 2 0 0116.138 21H7.862a2 2 0 01-1.995-1.858L5 7m5 4v6m4-6v6m1-10V4a1 1 0 00-1-1h-4a1 1 0 00-1 1v3M4 7h16"/></svg></button>
                        </div>
                    </td>
                </tr>
                @empty
                <tr><td colspan="6" class="px-4 py-10 text-center text-slate-400">No statuses found.</td></tr>
                @endforelse
            </tbody>
        </table>
    </div>
    ...
    <div x-show="showModal" x-cloak class="fixed inset-0 z-50 flex items-center justify-center px-4">
        ...
        <div class="relative bg-white rounded-xl shadow-xl w-full max-w-md p-6" @click.stop>
            <div class="flex items-center justify-between mb-5">
                <h3 class="text-base font-bold text-slate-800" x-text="mode==='create' ? 'Add Status' : 'Edit Status'"></h3>
                ...
                <div class="space-y-4">
                    ...
                    <div>
                        ...
                        <select x-model="form.color_class" required class="w-full px-3 py-2 text-sm border border-slate-300 rounded-lg focus:ring-2 focus:ring-teal-500 outline-none">
                            <option value="slate">Slate (Grey)</option><option value="blue">Blue</option><option value="green">Green</option><option value="yellow">Yellow</option><option value="red">Red</option><option value="purple">Purple</option><option value="orange">Orange</option>
                        </select>
                    </div>
                    <div><label class="block text-xs font-medium text-slate-700 mb-1">Sort Order <span class="text-red-500">*</span></label><input type="number" min="0" x-model="form.sort_order" required class="w-full px-3 py-2 text-sm border border-slate-300 rounded-lg focus:ring-2 focus:ring-teal-500 outline-none"></div>
    ...
    <div x-show="showDelete" x-cloak class="fixed inset-0 z-50 flex items-center justify-center px-4">
        ...
        <div class="relative bg-white rounded-xl shadow-xl w-full max-w-sm p-6 text-center" @click.stop>
            ...
            <h3 class="text-base font-bold text-slate-800 mb-1">Delete Status?</h3>
