...
<style>
    .ts-control { border-radius: 0.5rem; border-color: #cbd5e1; padding: 0.5rem 0.75rem; font-size: 0.875rem; line-height: 1.25rem; min-height: 38px; box-shadow: none; outline: none; transition: all 0.2s; }
    .ts-control.focus { border-color: #14b8a6; box-shadow: 0 0 0 2px rgba(20, 184, 166, 0.2); }
    .ts-wrapper.single .ts-control:after { right: 1rem; border-color: #94a3b8 transparent transparent transparent; border-width: 5px 4px 0 4px; }
    .ts-dropdown { border-radius: 0.5rem; border-color: #cbd5e1; font-size: 0.875rem; box-shadow: 0 4px 6px -1px rgb(0 0 0 / 0.1); }
    .ts-dropdown .option { padding: 0.5rem 0.75rem; white-space: nowrap; overflow: hidden; text-overflow: ellipsis; }
    .ts-control > .item { white-space: nowrap; overflow: hidden; text-overflow: ellipsis; max-width: calc(100% - 20px); display: block; }
</style>
...
<div x-data="{
    ...
    formData: {
        ...
        test_status: '', testing_details: '', remark: ''
    },
    selectedId: null,
    activeTab: 'general',
    showFilters: false,
        ...
        <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4 items-end">
            ...
            <div>
                ...
                <div x-id="['filter-status-select']">
                    <select :id="$id('filter-status-select')" x-model="filters.status_id" x-init="setTimeout(() => { let ts = new TomSelect($el, { create: false, placeholder: 'All' }); $watch('filters.status_id', val => ts.setValue(val, true)); ts.on('change', val => filters.status_id = val); }, 100)" class="w-full" placeholder="All">
                        <option value="">All</option>
            ...
            <div>
                ...
                <div x-id="['filter-test-status-select']">
                    <select :id="$id('filter-test-status-select')" x-model="filters.test_status" x-init="setTimeout(() => { let ts = new TomSelect($el, { create: false, placeholder: 'All' }); $watch('filters.test_status', val => ts.setValue(val, true)); ts.on('change', val => filters.test_status = val); }, 100)" class="w-full" placeholder="All">
                        <option value="">All</option>
        ...
        <table class="w-full text-sm">
            ...
            <tbody class="divide-y divide-slate-100">
                ...
                <tr class="hover:bg-slate-50">
                    <td class="px-4 py-3 text-slate-600 whitespace-nowrap">{{ $log->log_date->format('d M Y') }}</td>
                    <td class="px-4 py-3">
                        <p class="font-medium text-slate-800">{{ $log->title }}</p>
                        @if($log->remark)<p class="text-xs text-slate-400 mt-0.5">{{ Str::limit($log->remark, 40) }}</p>@endif
                    </td>
                    ...
                    <td class="px-4 py-3">
                        @if($log->test_status)
                        <span class="px-2 py-0.5 rounded text-xs font-medium bg-indigo-50 text-indigo-700">{{ $log->test_status }}</span>
                        @else<span class="text-slate-400">—</span>@endif
                    </td>
                    <td class="px-4 py-3">
                        <span class="px-2 py-0.5 text-xs rounded-full font-medium bg-{{ $log->status->color_class }}-100 text-{{ $log->status->color_class }}-700">{{ $log->status->name }}</span>
                    </td>
                    <td class="px-4 py-3">
                        <div class="flex items-center gap-1">
                            <button @click="openEdit({{ $log->id }})" title="Edit" class="p-1 text-slate-400 hover:text-blue-600 rounded">
                                <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M11 5H6a2 2 0 00-2 2v11a2 2 0 002 2h11a2 2 0 002-2v-5m-1.414-9.414a2 2 0 112.828 2.828L11.828 15H9v-2.828l8.586-8.586z"/></svg>
                            </button>
                            <button @click="openFeedback({{ $log->id }})" title="Feedback" class="p-1 text-slate-400 hover:text-yellow-600 rounded">
                                <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M8 10h.01M12 10h.01M16 10h.01M9 16H5a2 2 0 01-2-2V6a2 2 0 012-2h14a2 2 0 012 2v8a2 2 0 01-2 2h-5l-5 5v-5z"/></svg>
                            </button>
                            <button @click="confirmDelete({{ $log->id }})" title="Delete" class="p-1 text-slate-400 hover:text-red-600 rounded">
                ...
                @empty
                <tr><td colspan="10" class="px-4 py-12 text-center text-slate-400">No work logs found. Click "Add Work Log" to create your first entry.</td></tr>
                @endforelse
            </tbody>
        </table>
    </div>
    ...
    <div x-show="showDelete" class="fixed inset-0 z-50 overflow-y-auto" style="display: none;">
        <div class="flex items-center justify-center min-h-screen px-4 py-8">
            <div class="fixed inset-0 bg-slate-900/50 backdrop-blur-sm transition-opacity" @click="showDelete = false"></div>
            <div class="relative bg-white rounded-xl shadow-2xl max-w-sm w-full p-6 text-center transform transition-all">
                ...
                <p class="text-sm text-slate-500 mb-6">Are you sure you want to delete this work log? This action cannot be undone.</p>
                <div class="flex gap-3">
                    <button @click="showDelete = false" class="flex-1 px-4 py-2 bg-white border border-slate-300 text-slate-700 font-medium rounded-lg hover:bg-slate-50 transition-colors">Cancel</button>
                    <button @click="deleteLog()" class="flex-1 px-4 py-2 bg-red-600 text-white font-medium rounded-lg hover:bg-red-700 transition-colors">
    ...
    <div x-show="showModal" class="fixed inset-0 z-50 overflow-y-auto" style="display: none;">
        <div class="flex items-center justify-center min-h-screen px-4 text-center sm:p-0">
            <div class="fixed inset-0 transition-opacity bg-slate-900/50 backdrop-blur-sm" @click="showModal = false"></div>
            <div class="relative inline-block w-full max-w-4xl overflow-hidden text-left align-bottom transition-all transform bg-white rounded-xl shadow-xl sm:my-8 sm:align-middle">
                
                <div class="px-6 py-4 border-b border-slate-200 bg-slate-50 flex justify-between items-center">
                    <h3 class="text-lg font-bold text-slate-800" x-text="modalMode === 'create' ? 'Add New Work Log' : 'Edit Work Log'"></h3>
                    <button type="button" @click="showModal = false" class="text-slate-400 hover:text-red-500 transition-colors">
                ...
                <div class="px-6 pt-3 flex gap-6 border-b border-slate-200 bg-slate-50/50">
                    <button type="button" @click="activeTab = 'general'" :class="activeTab === 'general' ? 'border-teal-500 text-teal-700' : 'border-transparent text-slate-500 hover:text-slate-700 hover:border-slate-300'" class="pb-2 text-sm font-medium border-b-2 transition-colors">1. General Info</button>
                    <button type="button" @click="activeTab = 'status'" :class="activeTab === 'status' ? 'border-teal-500 text-teal-700' : 'border-transparent text-slate-500 hover:text-slate-700 hover:border-slate-300'" class="pb-2 text-sm font-medium border-b-2 transition-colors">2. Status, Priority & Testing</button>
                    <button type="button" @click="activeTab = 'metrics'" :class="activeTab === 'metrics' ? 'border-teal-500 text-teal-700' : 'border-transparent text-slate-500 hover:text-slate-700 hover:border-slate-300'" class="pb-2 text-sm font-medium border-b-2 transition-colors">3. Metrics & Duration</button>
                </div>
                    ...
                    <form @submit.prevent="submitForm" novalidate>
                            ...
                            <div class="grid grid-cols-1 gap-5 md:grid-cols-2">
                                ...
                                <div>
                                    ...
                                    <div x-id="['sub-kra-select']">
                                        <select :id="$id('sub-kra-select')" x-model="formData.sub_kra_id" x-init="setTimeout(() => { let ts = new TomSelect($el, { create: false, placeholder: 'Select Sub-KRA' }); $watch('formData.sub_kra_id', val => ts.setValue(val, true)); ts.on('change', val => formData.sub_kra_id = val); }, 100)" class="w-full" placeholder="Select Sub-KRA">
                            ...
                            <div class="grid grid-cols-1 gap-5 md:grid-cols-2">
                                ...
                                <div>
                                    ...
                                    <div x-id="['status-select']">
                                        <select :id="$id('status-select')" x-model="formData.status_id" x-init="setTimeout(() => { let ts = new TomSelect($el, { create: false, placeholder: 'Select Status' }); $watch('formData.status_id', val => ts.setValue(val, true)); ts.on('change', val => formData.status_id = val); }, 100)" class="w-full" placeholder="Select Status">
                                ...
                                <div>
                                    ...
                                    <div x-id="['priority-select']">
                                        <select :id="$id('priority-select')" x-model="formData.priority_id" x-init="setTimeout(() => { let ts = new TomSelect($el, { create: false, placeholder: 'Select Priority' }); $watch('formData.priority_id', val => ts.setValue(val, true)); ts.on('change', val => formData.priority_id = val); }, 100)" class="w-full" placeholder="Select Priority">
                                ...
                                <div>
                                    ...
                                    <div x-id="['test-status-select']">
                                        <select :id="$id('test-status-select')" x-model="formData.test_status" x-init="setTimeout(() => { let ts = new TomSelect($el, { create: false, placeholder: 'N/A' }); $watch('formData.test_status', val => ts.setValue(val, true)); ts.on('change', val => formData.test_status = val); }, 100)" class="w-full" placeholder="N/A">
                                ...
                                <div class="md:col-span-2">
                                    ...
                                    <textarea x-model="formData.testing_details" rows="2" class="w-full px-3 py-2 text-sm border border-slate-300 rounded-lg focus:ring-2 focus:ring-teal-500 outline-none transition-all placeholder:text-slate-400" placeholder="Details regarding testing (logs, issues, outcomes...)"></textarea>
                                </div> 
                                <div class="md:col-span-2">
                                    <label class="block text-xs font-medium text-slate-700 mb-1">Remark</label>
                                    <input type="text" x-model="formData.remark" class="w-full px-3 py-2 text-sm border border-slate-300 rounded-lg focus:ring-2 focus:ring-teal-500 outline-none transition-all placeholder:text-slate-400" placeholder="Special note or remark">
                                </div>
                            </div>
                        </div>
                        ...
                        <div x-show="activeTab === 'metrics'" x-transition:enter="transition ease-out duration-300" x-transition:enter-start="opacity-0 translate-y-2" x-transition:enter-end="opacity-100 translate-y-0" style="display: none;">
                            <div class="grid grid-cols-1 gap-5 md:grid-cols-2">
                                ...
                                <div>
                                    <label class="block text-xs font-medium text-slate-700 mb-1">Actual Duration (Hours)</label>
                                    <input type="number" step="0.01" x-model.number="formData.actual_duration" class="w-full px-3 py-2 text-sm border border-slate-300 rounded-lg focus:ring-2 focus:ring-teal-500 outline-none transition-all">
                                </div>
                                <div>
                        ...
                        <div class="mt-8 flex justify-end gap-3 pt-4 border-t border-slate-100">
                            <button type="button" x-show="activeTab !== 'general'" @click="activeTab = activeTab === 'metrics' ? 'status' : 'general'" class="px-5 py-2 text-sm font-medium text-slate-600 bg-white border border-slate-300 rounded-lg hover:bg-slate-50 transition-colors mr-auto">Back</button>
                            
                            <button type="button" @click="showModal = false" class="px-5 py-2 text-sm font-medium text-slate-600 bg-white border border-slate-300 rounded-lg hover:bg-slate-50 transition-colors">Cancel</button>
                            
                            <button type="button" x-show="activeTab !== 'metrics'" @click="activeTab = activeTab === 'general' ? 'status' : 'metrics'" class="px-5 py-2 text-sm font-medium text-white bg-slate-800 rounded-lg hover:bg-slate-900 transition-colors">Next Step</button>
                            
                            <button type="submit" x-show="activeTab === 'metrics'" class="px-5 py-2 text-sm font-medium text-white bg-teal-600 rounded-lg hover:bg-teal-700 transition-colors flex items-center gap-2" :disabled="loading">
                                <svg x-show="loading" class="animate-spin -ml-1 mr-1 h-4 w-4 text-white" fill="none" viewBox="0 0 24 24" style="display: none;"><circle class="opacity-25" cx="12" cy="12" r="10" stroke="currentColor" stroke-width="4"></circle><path class="opacity-75" fill="currentColor" d="M4 12a8 8 0 018-8V0C5.373 0 0 5.373 0 12h4zm2 5.291A7.962 7.962 0 014 12H0c0 3.042 1.135 5.824 3 7.938l3-2.647z"></path></svg>
    ...
    <div x-show="showFeedback" class="fixed inset-0 z-50 overflow-y-auto" style="display: none;">
        <div class="flex items-center justify-center min-h-screen px-4 text-center sm:p-0">
            <div class="fixed inset-0 transition-opacity bg-slate-900/50 backdrop-blur-sm" @click="showFeedback = false"></div>
            <div class="relative inline-block w-full max-w-2xl overflow-hidden text-left align-bottom transition-all transform bg-white rounded-xl shadow-xl sm:my-8 sm:align-middle p-6">
                    ...
                    <template x-for="fb in feedbacks" :key="fb.id">
                        <div class="mb-3 p-3 bg-white border border-slate-200 rounded-lg shadow-sm">
                            <div class="flex justify-between items-center mb-1">
                                <span class="text-xs font-bold font-medium" :class="fb.feedback_type == 'self' ? 'text-blue-600' : 'text-purple-600'" x-text="fb.feedback_type == 'self' ? 'Self Review' : 'Manager Review'"></span>
                                <span class="text-xs text-slate-400" x-text="new Date(fb.created_at).toLocaleDateString()"></span>
                            </div>
                ...
                <form @submit.prevent="submitFeedback" class="border-t border-slate-100 pt-4">
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                        <div>
                            ...
                            <select x-model="feedbackForm.feedback_type" required class="w-full px-3 py-2 text-sm border border-slate-300 rounded-lg">
                        ...
                        <div>
                            ...
                            <input type="number" min="1" max="5" x-model="feedbackForm.rating" required class="w-full px-3 py-2 text-sm border border-slate-300 rounded-lg">
                        </div>
                        <div class="md:col-span-2">
                            <label class="block text-xs font-medium text-slate-600 mb-1">Comment</label>
                            <textarea x-model="feedbackForm.comment" required rows="2" class="w-full px-3 py-2 text-sm border border-slate-300 rounded-lg placeholder:text-slate-400" placeholder="Your thoughts on this task..."></textarea>
                        </div>
                    </div>
                    <div class="mt-4 flex justify-end">
                        <button type="submit" class="px-5 py-2 text-sm font-medium text-white bg-indigo-600 rounded-lg hover:bg-indigo-700 flex items-center gap-2" :disabled="loading">
                            <svg x-show="loading" class="animate-spin -ml-1 mr-1 h-4 w-4 text-white" fill="none" viewBox="0 0 24 24" style="display: none;"><circle class="opacity-25" cx="12" cy="12" r="10" stroke="currentColor" stroke-width="4"></circle><path class="opacity-75" fill="currentColor" d="M4 12a8 8 0 018-8V0C5.373 0 0 5.373 0 12h4zm2 5.291A7.962 7.962 0 014 12H0c0 3.042 1.135 5.824 3 7.938l3-2.647z"></path></svg>
